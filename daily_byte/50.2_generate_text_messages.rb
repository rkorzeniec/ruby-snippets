...
class Solution
  ...
  def messages(digits, combination: '', result: [], i: 0)
    return result.append(combination.dup) if i - 1 == digits.size

    MAPPING[digits[i]].chars.each do |char|
      next if char.nil?

      messages(digits[i..], combination: combination + char, result:, i: i + 1)
    end
